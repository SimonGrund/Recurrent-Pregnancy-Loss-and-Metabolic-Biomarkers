...
d = filter(d, patient_type %in% c(0,3))
...
d = dplyr::select(d,
  ...
  mutate(
    'HbA1c>38' = ifelse(hba1c >= 38,"Elevated", "Normal"),
...
#%>%
 # pivot_longer(c('HbA1c>38', 'Glucose>6,4', 'Total cholesterol>5',
  #               'LDL>3', 'HDL<1,2', 'Triglycerides>2'))

d$`HbA1c>38` = factor(d$`HbA1c>38`, levels = rev(c("Elevated", "Normal")))
...
#HBA1C
gg = glm('group ~ BMI + `HbA1c>38`', family = binomial, data = d)
summary(gg)
...
out_tmp = data.frame(feature = "HbA1c>38", 
...
summary = d%>%
  ...
  na.omit(`HbA1c>38`)%>%
  mutate(n = n(),
         n_elevated = sum(`HbA1c>38` == "Elevated"),
         rate = n_elevated / n)%>%
...
out2$feature = factor(out2$feature, levels = c('HbA1c>38', 'Glucose>6,4', 'Total cholesterol>5',
                                   'LDL>3', 'HDL<1,2', 'Triglycerides>2'))
...
ggplot(out2, aes(x = name, y = value))+
  ...
  ggtitle("Recurrent late pregnancy loss")


ggsave("../Results/Figures/Figure 4/Third_RPL_Adjusted_Odds_ratio_biomarkers__above_treshhold.pdf",
