...
ggplot(out2, aes(x = name, y = value))+
  ...
  ggtitle("Secondary RPL")


ggsave("../Results/Figures/Figure 4/Secondary_RPL_Adjusted_Odds_ratio_biomarkers__above_treshhold.pdf",
...
quartz(type = 'pdf', file = "../Results/Figures/Figure 4/secondary_RPL.pdf", width = 4, height = 3)
...
ggplot(pd2, aes(x = odds_ratio_glm, y = feature))+
  ...
  ylab("")+
  xlab("Odds-ratio")+
  ggtitle(label = "Secondary RPL, adjusted")
dev.off()  
...
write.table(out3, "../Results/Tables/TMP/or2_ad.tsv", sep = "\t", col.names = T, row.names = F)
