...
d = filter(d, patient_type %in% c(0,1))
...
ggplot(out2, aes(x = name, y = value))+
  ...
  ggtitle("Primary RPL, unadjusted for baseline characteristics")

ggsave("../Results/Supp Figures/Supp Figure 3//Odds_ratio_biomarkers_above_treshhold_primary_RPL.pdf",
       width = 5, height = 4)
...
write.table(out3, "../Results/Tables/TMP/or1_un.tsv", sep = "\t", col.names = T, row.names = F)
