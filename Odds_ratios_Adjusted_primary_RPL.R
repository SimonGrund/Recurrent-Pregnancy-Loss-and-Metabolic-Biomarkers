...
ggsave("../Results/Figures/Figure 4/primary_RPL_Adjusted_Odds_ratio_biomarkers__above_treshhold.pdf",
...
quartz(type = 'pdf', file = "../Results/Figures/Figure 4/primary_RPL.pdf", width = 4, height = 3)
...
ggplot(pd2, aes(x = odds_ratio_glm, y = feature))+
  ...
  ylab("")+
  xlab("Odds-ratio")+
  ggtitle(label = "Primary RPL, adjusted")
dev.off()  
...
out3 = out3%>%
  mutate(
    "Patient group" = "Primary RPL",
    "Adjusted for baseline characteristica" = "Yes"
  )

write.table(out3, "../Results/Tables/TMP/or1_ad.tsv", sep = "\t", col.names = T, row.names = F)
